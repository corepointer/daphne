...
namespace CUDA::Convolution {
            ...
            const size_t stride_w, const size_t pad_h, const size_t pad_w, DCTX(dctx))
    {
        ...
        CHECK_CUDNN(cudnnSetTensor4dDescriptor(ctx->src_tensor_desc, ctx->tensor_format,
                ctx->template getCUDNNDataType<VT>(), batch_size, num_channels, img_h, img_w));
        const int tensorDims = 4;
        int tensorOuputDimA[tensorDims];
        const int filterDimA[tensorDims] = {static_cast<int>(F), static_cast<int>(num_channels),
...
//        }
        ...
        if (ctx->conv_algorithm < 0) {
            ...
            CHECK_CUDNN(cudnnFindConvolutionForwardAlgorithm(ctx->getCUDNNHandle(), ctx->src_tensor_desc,
                    ctx->filter_desc, ctx->conv_desc, ctx->dst_tensor_desc, requestedAlgoCount, &returnedAlgoCount,
                    results));
            algo = results[0].algo;
        ...
        CHECK_CUDNN(cudnnGetConvolutionForwardWorkspaceSize(ctx->getCUDNNHandle(), ctx->src_tensor_desc,
                ctx->filter_desc, ctx->conv_desc, ctx->dst_tensor_desc, algo, &workspace_sizeInBytes));
        ...
        if(bias) {
            if (bias != filter) {
                ...
                blend_beta = 1;
                CHECK_CUDNN(cudnnAddTensor(ctx->getCUDNNHandle(), &blend_alpha, ctx->src_tensor_desc, d_bias,
